...
module.exports = {
  ...
  getRateReconcilation: async (req, res) => {
    try {
      ...
      let amount = 0;
      let data = {};
      let typesOfCharge = await RatePerKgPerBoxHelper.calculateOrderAdditinalServiceAmount(requests.orderId)
      if (requests.rateType == 1) {
        ...
        data = {
          amount: Number(parseFloat(amount).toFixed(2)) + Number(typesOfCharge),
        };
      } else {
        ...
        if (data.status == 1) {
          ...
          data = {
            amount: Number(amount) + Number(typesOfCharge),
          };
      ...
      return success(res, "Rate reconcilation amount", data);
    } catch (error) {
  ...
  createRateReconcilation: async (req, res) => {
    try {
      ...
      if (requests.rateType == 1) {
        ...
        if (!amount)
          return response(res, 422, "Vendor not accept shipment weight");
        await calculateFinalPriceForRatePerKgAndCreate(
          order,
          items,
          requests,
          req.decodedData.id
        ...
        data = await calculateFinalPriceForRatePerBox(order, items, requests);
        if (data.status == 1) {
          await calculateFinalPriceForRatePerBoxAndCreate(
            order,
            items,
            requests,
            req.decodedData.id
  ...
  getRateReconcilationDashboard: async (req, res) => {
    try {
      ...
      results.forEach((result) => {
        ...
        if (status == 4 || status == 5 || status == 2 || status == 3)
          data.totalClosedDescrepancy += 1;
  ...
  acceptRejectRateReconcilation: async (req, res) => {
    try {
      ...
      requests.userType = "admin";
      let exist = await OrderWeightReconcilation.findOne({
        where: {
          id: requests.id,
        },
      });
      if (!exist) return response(res, 422, "Weight reconcilation not found");
      ...
      await RatePerKgPerBoxHelper.acceptRejectWeightReconcilation(
        req,
        requests,
        order,
        exist
...
async function calculateFinalPriceForRatePerKg(order, items, requestData) {
  ...
  let additionalAddedCharge =
    requestData.unit == "gm"
      ? requestData.additionalWeight / 1000
      : requestData.additionalWeight;
  let data = order;
  data.items = items;
  data.shipmentAmount = order.Shipment_value;
  data.shipmentWeight =
    parseFloat(order.Shipment_weight) + parseFloat(additionalAddedCharge);
  data.boxes = items.reduce((sum, item) => sum + item.boxes, 0);
  let perboxInWeight = parseFloat(additionalAddedCharge / data.boxes).toFixed(
    2
  );
  data.items = items.map((item) => ({
    ...
    boxes: item.boxes,
    unit: item.unit,
    boxWeight: Number(item.boxWeight) + Number(perboxInWeight),
  }));
      ...
      if (getSourceService && getDestinationService) {
        ...
        if (tat) {
          ...
          if (cargoRate) {
            ...
            if (vendorRate) {
              let vservice = await RatePerKgPerBoxHelper.getRatePerKgServiceList(vendorRate, cargoRate, data.items, data.shipmentWeight, ODA1, ODA2, ODA3, ODA11, ODA22, ODA33, data.shipmentAmount, getServiceList[i], null, markup, getSourcePincodeId, getDestinationPincodeId, TAT)

              if (vservice) {

                newFinalAmount = parseFloat(vservice?.totalAmount).toFixed(2) - parseFloat(data?.totalAmount).toFixed(2)
              }
        ...
        if (tat) {
          ...
          if (cargoRate) {
            ...
            if (vendorRate) {
              ...
              if (vservice) {
                newFinalAmount = parseFloat(vservice?.totalAmount).toFixed(2) - parseFloat(data?.totalAmount).toFixed(2)
                return { status: 1, amount: parseFloat(newFinalAmount).toFixed(2) };
              }
...
async function calculateFinalPriceForRatePerKgAndCreate(
  order,
  items,
  requestData,
  creatorId
        ...
        if (tat) {
          ...
          if (cargoRate) {
            ...
            for (
              ...
              if (itemUnit == "FEET") {
                volumatricShipingWeight +=
                  (data.items[itemIndex].L *
                    data.items[itemIndex].B *
                    data.items[itemIndex].H *
                    data.items[itemIndex].boxes *
                    30.4 *
                    CFT) /
                  cargoRate.dividend;
            ...
            let finalShipmentWeight =
              data.shipmentWeight > volumatricShipingWeight
                ? data.shipmentWeight
                : volumatricShipingWeight;
            console.log(finalShipmentWeight, "finalShipmentWeight");

            if (
              parseFloat(finalShipmentWeight) <= parseFloat(cargoRate.cwMax)
            ) {
              //     console.log("Qweqwe");
              ...
              if (additionalCharge.length) {
                ...
                for (let j = 0; j < additionalCharge.length; j++) {
                  if (
                    additionalCharge[j] &&
                    additionalCharge[j].chargesType
                  ) {
                ...
                for (let j = 0; j < additionalCharge.length; j++) {
                  if (
                    ...
                    if (
                      ...
                      let amount =
                        await CalculativeHelper.evaluateLeftToRight(
                          finalFormula
                        );
                      amount =
                        charge.minValue &&
              ...
              let finaldata = {
                ...
                rate:
                  parseFloat(vendorRate.rates) +
                  (parseFloat(vendorRate.rates) * parseFloat(markup)) / 100, //zone to zone rate(per kg),
                ...
                remark: requestData.remark
                  ...
                  : JSON.stringify([
                    { userId: creatorId, userType: "admin", remark: "" },
                  ]),
              };
...
async function calculateFinalPriceForRatePerBoxAndCreate(
  order,
  items,
  requestData,
  creatorId
